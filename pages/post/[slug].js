...
import postPageInitialPropsTqHtResource from '../../resources/post-page-initial-props-tq_ht'
import postPageInitialPathsTqImResource from '../../resources/post-page-initial-paths-tq_im'
...
export async function getStaticProps(context) {
  try {
    ...
    const response = await postPageInitialPropsTqHtResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await postPageInitialPathsTqImResource({
      content_type: 'post',
      select: 'fields.slug',
    })
    return {
