...
import authorPageInitialPropsTq7bResource from '../../resources/author-page-initial-props-tq_7b'
import authorPageInitialPathsTqLEResource from '../../resources/author-page-initial-paths-tq_l-e'
...
export async function getStaticProps(context) {
  try {
    const response = await authorPageInitialPropsTq7bResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await authorPageInitialPathsTqLEResource({
      content_type: 'author',
      select: 'fields.name',
    })
    return {
